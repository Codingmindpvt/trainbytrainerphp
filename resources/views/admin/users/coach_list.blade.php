                ...
                <div class="tab-pane fade active in" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab">
                    <div class="table-responsive">
                        <table width="100%" cellspacing="0" cellpadding="0" class="myDataTable">
                            ...
                            <tbody>
                                <?php
                                $i  = ($coach_users->perPage() * ($coach_users->currentPage() - 1)) + 1;
                                ?>
                                @foreach( $coach_users as $serial_no=>$coach )
                                <tr>
                                    <td>{{ $i++}}</td>
                                    ...
                                    <td>
                                        ...
                                        <input type="checkbox" class="toggle-status-class" data-id="{{$coach->id}}"
                                            data-toggle="toggle" data-on="Active" data-off="Suspend"
                                            data-onstyle="success" data-offstyle="danger"
                                            {{ $coach->status == 'A' ? 'checked' : '' }}>

                                    </td>
                                    <td>
                                        @if(empty($coach['coach_detail']['status']) &&
                                        !isset($coach['coach_detail']['status']))
                                        {{"Not Submitted"}}
                                        @elseif($coach['coach_detail']['status'] == 'P')
                                        <button type="button" title="Verified" value="V" id="{{$coach->id}}"
                                            class="btn btn-success verify-coach-profile abc_verify_{{$coach->id}}">
                                            <i class="fa fa-check" aria-hidden="true"></i>
                                        </button>
                                        <button type="button" title="Rejected"
                                            onclick="return confirm('Are you sure you want to suspend the user account?')"
                                            data-id="{{$coach->id}}"
                                            class="btn btn-danger reject-coach-profile abc_reject_{{$coach->id}}">
                                    ...
                                    <td>
                                        ...
                                        @if(empty($coach['verification_detail']['badge_status']) &&
                                        !isset($coach['verification_detail']['badge_status']))
                                        {{"Not Submitted"}}
                                        @elseif($coach['verification_detail']['badge_status'] == 'P')
                                        <button type="button" title="Certify" value="C" id="{{$coach->id}}"
                                            class="btn btn-success certifyBadgeProfile abc_certified_{{$coach->id}}">
                                            <i class="fa fa-check" aria-hidden="true"></i>
                                        </button>

                                        <button type="button"
                                            onclick="return confirm('Are you sure you want to reject this badge request of coach?')"
                                            title="Reject" data-id="{{$coach->id}}"
                                            class="btn btn-danger reject-badge-profile abc_reject_badge_{{$coach->id}}">
                                            <i class="fa fa-close" aria-hidden="true"></i>
                                        </button>
                                        @else
                                    ...
                                    <td class="text-underline"><b><a href="{{ route('admin.Program.list',$coach->id) }}">View</a></b></td>
                                    <td class="text-underline"><b><a href="{{ route('admin.Class.list',$coach->id) }}">View</a></b></td>
                                    <td class="text-underline"><b><a href="{{ route('admin.review.list',$coach->id) }}">View</a></b></td>
                                    <td>
                                        <a href="{{ route('admin.coach.detail',$coach->id) }}" data-toggle="tooltip"
                                            data-placement="top" title="user.detail" class="action_btn" alt="detail"><i
                                                class="fa fa-eye" aria-hidden="true"></i></a>
                                        <a href="{{ route('admin.user.editCoach',$coach->id) }}" data-toggle="tooltip"
