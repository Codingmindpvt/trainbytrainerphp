            ...
            <div class="col-md-8">
                <div class="user-profle-right-side">
                    ...
                    <div class="order-id-purchase">
                        <p><span>Order ID:</span> {{ isset($program['order_id']) ? "#".$program['order_id'] : "-----"}}
                        </p>
                        <p><span>Purchased on:</span>
                            {{ isset($program['created_at']) ? date_format(date_create($program->created_at),"M d, Y H:i A") : "-----"}}
                        </p>
                    </div>
                    <div class="details-order-image">
                        @if(!empty($program['program']['image_file']))
                        <img src="{{asset('public/'.$program['program']['image_file'])}}" alt="" class="mr-3">
                        @else
                        <img src="{{asset('public/images/default-image-file-o.png')}}" alt="" class="mr-3">
                        @endif

                        <div class="detail-order-content">
                            <h2>{{ isset($program['program']['program_name']) ? $program['program']['program_name'] : "-----"}}
                            </h2>
                            <h5>{{isset($program['program']['program_name']) ? DEFAULT_CURRENCY.$program['program']['price'] : "-----"}}<span>
                                    {{isset($program['payment']['user']['first_name']) ? ucwords($program['payment']['user']['first_name']." ".$program['payment']['user']['last_name']) : "-----"}}</span>
                            </h5>
                            ...
                            <p>{{isset($program['program']['description']) ? $program['program']['description'] : "-----"}}
                            </p>
                        </div>
                    </div>
                    <div class="product-downloadable-area">
                        ...
                        @php
                        $documents = isset($program['program']['program_image']) ? $program['program']['program_image']
                        :
                        [];
                        @endphp
                        ...
                        <div class="row">
                            ...
                            <div class="offset-6 col-md-6">
                                <div class="total-amount-area billing-payment-area">
                                    ...
                                    <div class="cart-total">
                                        <p>Tax(5%)</p>
                                        <p>{{ isset($program['program']['price']) ? DEFAULT_CURRENCY.$program['payment']->getTaxAmount() : "-----"}}
                                        </p>
                                    </div>
                                    <hr>
                                    <div class="cart-total">
